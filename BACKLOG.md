...
- ~~**Status item icon frame-swap animation.**~~ **Second asset shipped 2026-04-18** — `StatusBarIconListening.png` + `@2x` now ship alongside `StatusBarIcon.png`. `StatusItemController.updateStatusItemAppearance(for:)` swaps between the two poses on recording / transcribing. Pure pose-swap, no cross-fade animation — good enough for dogfood.
- **macOS 15 URL-scheme branching for permission deep-links** — *fix if it breaks*. `StatusItemController` uses the macOS 14 anchor (`Privacy_Microphone`, `Privacy_ListenEvent`) which still works on macOS 15 as of 2026-04-18. If Apple renames the anchor in a future point release, branch via `if #available(macOS 15, *)`. No preemptive fix — just react.
...
- ~~**`.icns` app icon.** Plan line 339 flagged for "after logo stabilises."~~ **Done `41bb6eb`** — hand-authored RGBA `AppIcon.icns` checked in at `Sources/SeshatAppKit/Resources/`, `package.sh` copies to `Seshat.app/Contents/Resources/Seshat.icns`.
- **`SeshatLogoView` pixel-fidelity trace.** Current SwiftUI `QuillShape` is a stylised vector, not pixel-matching the master logo. Manus is working on an SVG trace (2026-04-18); convert to SwiftUI Path once received so the live pill quill matches the `.icns` master.
- **Stable signing identity (`Nitkrar Dev`).** `scripts/package.sh` now prefers the `Nitkrar Dev` codesigning identity if present in the login Keychain (stable identity → TCC permissions persist across rebuilds), falling back to ad-hoc otherwise. One-time setup required per machine: Keychain Access → Certificate Assistant → Create a Certificate… → name `Nitkrar Dev`, Identity Type `Self Signed Root`, Certificate Type `Code Signing`, save in login keychain. Check via `security find-identity -v -p codesigning | grep "Nitkrar Dev"` — non-zero hit = working.
