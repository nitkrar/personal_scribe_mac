...
Layer 8 centralizes that read model into one `@MainActor` observable service. The service stays strictly downstream of transcript persistence, derives everything from the existing transcript store fields, and gives the eventual unified-window Home tab a direct binding surface for rollups plus the recent-transcriptions list.
...
- `Contract C2`: The live service is `@MainActor`, `ObservableObject`, and publishes rollups plus recent transcripts directly for UI binding.
...
- `Contract C5`: Home tab and any overlapping debug stats surface bind the same published snapshot. No consumer does its own weekly word-count or duration math once Layer 8 Stage 2 is complete.
...
- `MetricsService: ObservableObject, Sendable`
  Main-actor published surface:
...
### Errors
- No new public error enum. The live service should log read failures, preserve the last good snapshot, and surface empty-state or stale-data UI through published state rather than forcing Home-tab consumers to catch errors.

## Proposed live implementation
`SQLiteMetricsService` should live in `Sources/SeshatCore/Metrics/` as a `@MainActor final class` implementing `MetricsService`. Stage 1 keeps it self-contained by injecting a `databaseURL`, `NotificationCenter`, `Calendar`, and clock/reference-date provider. That lets the new layer build and test in isolation before Layer 2 Stage 2 rewires production composition around `StorageLocator`.
...
### Stage 1.2 - Implement the SQLite-backed rollup reader and published service
...
| Change | Before | After |
|---|---|---|
| Live implementation | `SQLiteTranscriptStore` exposes append/recent/count/search only (`Sources/SeshatCore/SQLiteTranscriptStore.swift:66-163`). | New `SQLiteMetricsReader` and `SQLiteMetricsService` query the same `transcripts.sqlite` read-only and publish rollups plus the recent list. |
...
### Stage 2.1 - Swap the unified-window Home tab to bind `MetricsService`
