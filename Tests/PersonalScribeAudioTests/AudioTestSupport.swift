...
// Private test-only helpers — do NOT expose from PersonalScribeAudioTests.
...
/// Reads PersonalScribeLogger messages emitted in the current process via OSLogStore.
enum LogProbe {
    static func audioMessages(since windowSeconds: TimeInterval = 30) throws -> [String] {
        ...
        let predicate = NSPredicate(format: "subsystem == %@ AND category == %@",
                                    PersonalScribeLogger.subsystem,
                                    PersonalScribeLogCategory.audio)
