...
- [ ] **MV-SI-19** With a warmed `whisper.cpp` model, verify non-status-item quit paths also fast-exit cleanly. First press `Cmd+Q` from the app's main menu. Then open **Advanced** → directory picker (`NSSavePanel`) and press `Cmd+Q` while the modal is visible. Both quits exit cleanly without the Metal teardown crash, and the prior launch's log includes `application_terminating_via_fast_exit`.
